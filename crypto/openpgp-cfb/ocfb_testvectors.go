...
func printTestVec(k []byte, prefix []byte, out []byte, resync packet.OCFBResyncOption, desc string) {
	fmt.Printf("\n// %s: %d-byte key (resync = %t)\n", desc, len(k), resync == packet.OCFBResync)
	fmt.Printf("key      = \"%x\"\n", k)
	fmt.Printf("prefix   = \"%x\"\n", prefix)
...
func ocfbTestVector(k []byte, iv []byte, resync packet.OCFBResyncOption) (out []byte, err error) {
	...
	// RFC4880-compliant output
	printTestVec(k, prefix, out, resync, "RFC4880")
	// Something guaranteed to be different.
	// with out[17:18] == ^out[17:18]
	out[17] = ^out[17]
	out[18] = ^out[18]
	printTestVec(k, prefix, out, resync, "should accept")
	// Zeroed 17:18.
	// with out[17:18] == 0
	out[17] = 0
	out[18] = 0
	printTestVec(k, prefix, out, resync, "should emit")
